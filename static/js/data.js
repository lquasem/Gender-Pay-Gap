...
function init() {
  ...
  d3.json("/byRace", function (dataSet) {
    // console.log(dataSet)
    var year = d3.select("#selDataset").property("value")
    // var categoryYear = Object.keys(dataSet.year).map(item => dataSet.year[item]);
    buildCharts(dataSet, year);
    // console.log(categoryYear)
  })
  // }

  function buildDtaSet(dataSet, year) {
    console.log("inside buildDataSet " + year)
    //console.log("Dataset")
    //console.log(dataSet)
    ...
    ).filter(row => row.get("year") === parseInt(year, 10))
